...
if (!immobilized) {
    ...
    var newLoc = new Vec2(x, y);
    
    var _distTravelled = newLoc.distance(prevLoc);
    if (sprint_modifier > 1) {
        statistic.distance_ran += _distTravelled;
    } else {
        statistic.distance_walked += _distTravelled;
    }
    ...
    if (_hor != 0 || _ver != 0) {
        ...
        if (_ver > 0 && _hor == 0) {
            ...
            hitbox.sprite_index = spr_player_collision_front;
            ...
            hitbox.sprite_index = spr_player_collision_side_right;
            change_orientation(Orientation.RIGHT);
        }
        else if (_hor < 0) {
            sprite_index = spr_player_walk_left;
            hitbox.sprite_index = spr_player_collision_side_left;
