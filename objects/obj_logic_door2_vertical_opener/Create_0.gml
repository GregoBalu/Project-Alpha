...
On = function(_inst_id) {
    tilemap_set_at_pixel(tilemap, wall_tile_id, x1, y1);
...
Off = function(_inst_id) {
    ...
    if (_coll != noone) {
        if (obj_player.x < center_horizontal) {
            //push left
            obj_player.x -= obj_player.bbox_right - left + 1;
        } else {
            //push right
            obj_player.x += right - obj_player.bbox_left + 1;
        }
    }
